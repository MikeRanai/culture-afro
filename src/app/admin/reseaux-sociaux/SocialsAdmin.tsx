...
import { Plus, Pencil, Trash2, X, Save, Eye, EyeOff, Share2 } from "lucide-react";
import { createSocialLink, updateSocialLink, deleteSocialLink } from "@/lib/actions";
...
const platforms = ["Instagram", "Facebook", "TikTok", "YouTube", "Twitter", "LinkedIn"];
const empty = { platform: "Instagram", handle: "", url: "", sortOrder: 0 };

export default function SocialsAdmin({ items }: { items: Item[] }) {
  ...
  function startEdit(item: Item) {
    setAdding(false);
    setEditing(item.id);
    setForm({ platform: item.platform, handle: item.handle, url: item.url, sortOrder: item.sortOrder });
  }
  ...
  function handleSave() {
    startTransition(async () => {
      if (adding) await createSocialLink(form);
      else if (editing) await updateSocialLink(editing, form);
      cancel();
    });
  }
  ...
  function handleToggle(item: Item) {
    startTransition(() => updateSocialLink(item.id, { active: !item.active }));
  }
  ...
  return (
    <>
      <div className="mb-6 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <h1 className="font-serif text-2xl font-bold text-afro-dark">Réseaux sociaux</h1>
          <p className="text-sm text-afro-dark/60">{items.length} réseau{items.length > 1 ? "x" : ""}</p>
        </div>
        {!showForm && (
      ...
      {showForm && (
        <div className="mb-6 rounded-2xl border border-afro-dark/10 bg-white p-5 shadow-sm">
          <h2 className="mb-4 text-sm font-bold uppercase tracking-wider text-afro-dark/40">
            {adding ? "Nouveau réseau" : "Modifier le réseau"}
          </h2>
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
            <div>
              <label className="mb-1 block text-sm font-medium text-afro-dark/70">Plateforme</label>
              <select
                value={form.platform}
            ...
            <div>
              <label className="mb-1 block text-sm font-medium text-afro-dark/70">Identifiant</label>
              <input
                type="text"
                value={form.handle}
                onChange={(e) => setForm({ ...form, handle: e.target.value })}
                className="min-h-[44px] w-full rounded-xl border border-afro-dark/10 bg-afro-light px-4 text-base text-afro-dark transition-colors focus:border-afro-orange focus:outline-none focus:ring-1 focus:ring-afro-orange/30"
                placeholder="@assocultureafro"
              />
            </div>
            <div className="sm:col-span-2">
              <label className="mb-1 block text-sm font-medium text-afro-dark/70">URL</label>
              <input
                type="url"
                value={form.url}
          ...
          <div className="mt-4 flex flex-wrap gap-3">
            <button onClick={handleSave} disabled={pending || !form.handle || !form.url} className="inline-flex min-h-[44px] items-center gap-2 rounded-xl bg-afro-orange px-5 py-2.5 text-sm font-semibold text-white shadow-sm transition-all hover:bg-afro-orange/90 disabled:opacity-50">
      ...
      {items.length === 0 ? (
        <div className="rounded-2xl border-2 border-dashed border-afro-dark/10 py-16 text-center">
          <Share2 className="mx-auto mb-3 h-12 w-12 text-afro-dark/20" />
          <p className="text-sm text-afro-dark/40">Aucun réseau social</p>
          ...
          {items.map((item) => (
            <div key={item.id} className={`group rounded-2xl bg-white p-4 shadow-sm sm:p-5 ${!item.active ? "opacity-60" : ""}`}>
              <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:gap-4">
                <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-afro-dark/5 text-sm font-bold text-afro-dark/60">
                  {item.platform.slice(0, 2).toUpperCase()}
                </div>
                <div className="min-w-0 flex-1">
                  <p className="text-sm font-bold text-afro-dark">{item.platform}</p>
                  <p className="text-sm text-afro-dark/60">{item.handle}</p>
                </div>
